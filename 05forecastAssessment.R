...
#do as plots (point forecast assessment)
...
assessmentPlotter1 <- function(metricMatrix,
                               ...
                               ylab,col=c("red","blue","darkgreen","cadetblue4","orange","black","darkred"),
                               panelName,
...
require(ggplot2)
storePlots <- list()
labs <- c("A","A","B","C","D","E","F","G")
for (i in 2:length(storeAssessments)){
...
ggsave(paste0(cd,"/out/forecast_plots/forecasts4.pdf"),g,width=9,height=5)
...
write.csv(crps,paste0(cd,"out/forecast_tables/crps_full.csv"))
write.csv(logs,paste0(cd,"out/forecast_tables/logs_full.csv"))
write.csv(dss,paste0(cd,"out/forecast_tables/dss_full.csv"))
